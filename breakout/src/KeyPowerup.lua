...
function KeyPowerup:init(image, x, y)
    self.image = image
    ...
    -- Adjusting the speed
    self.speed = 100
    -- Width and height of the key powerup
    self.width = 16
    self.height = 16
end

function KeyPowerup:reset()
...
function KeyPowerup:collides(paddle)
...
function KeyPowerup:render()
    love.graphics.draw(self.image, gFrames['key'][1], self.x, self.y)
end
